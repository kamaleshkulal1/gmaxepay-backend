...
const dthPlanFetch = async (req, res) => {
    try {
        ...
        const operator = await dbService.findOne(model.operator, { operatorType: "EXPRESS DTH", operatorCode: opcode });
...
const dthRecharge = async (req, res) => {
    try {
        ...
        const [existingUser, operator, existingCompany] = await Promise.all([
            ...
            dbService.findOne(model.operator, { operatorType: "EXPRESS DTH", operatorCode: opcode }),
