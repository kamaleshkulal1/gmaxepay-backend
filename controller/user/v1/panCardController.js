...
const panCardActions = async (req, res) => {
  try {
    ...
    // Get orderid from response (generated by service) or generate fallback
    const orderid = response.orderid || generateSystemReference();
    
    // Ensure orderid is not null (required field)
    if (!orderid) {
      return res.failure({ message: 'Failed to generate order ID' });
    }
