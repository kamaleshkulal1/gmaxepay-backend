...
const { autoGenerateDefaultSchemas } = require('../../admin/v1/companySchemaController');
...
const sendSmsMobile = async (req, res) => {
  try {
    ...
    // Auto-generate default Good/Bad schemas for Master Distributor and Distributor (no schemas for Retailer)
    try {
      if (userRole === 3 || userRole === 4) {
        const roleName = userRole === 3 ? 'MD' : 'DI';
        await autoGenerateDefaultSchemas(companyId, newUser.id, userRole, roleName);
      }
    } catch (schemaError) {
      console.error('Error auto-generating default schemas during onboarding:', schemaError);
      // Continue onboarding even if schema setup fails
    }
