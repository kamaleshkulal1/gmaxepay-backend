        ...
        if (isSuccess) {
            ...
            if ([4, 5].includes(user.userRole) && commData.users.companyAdmin) {
                ...
                if (commData.users.distributor && commData.wallets.distributorWallet && user.userRole === 5) {
                    ...
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.distributor.id, companyId: user.companyId, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - dist comm`, amount: amountNumber, comm: distCommAmt, surcharge: 0, openingAmt: dO, closingAmt: dC, credit: distNetAmt, debit: distShortfallAmt + distributorTDS, merchantTransactionId, transactionId, paymentStatus, distributorCom: distCommAmt, distributorComTDS: distributorTDS, addedBy: commData.users.distributor.id, updatedBy: commData.users.distributor.id }));
                }
                if (commData.users.masterDistributor && commData.wallets.masterDistributorWallet) {
                    ...
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.masterDistributor.id, companyId: user.companyId, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - md comm`, amount: amountNumber, comm: mdCommAmt, surcharge: 0, openingAmt: mO, closingAmt: mC, credit: mdNetAmt, debit: mdShortfallAmt + masterDistTDS, merchantTransactionId, transactionId, paymentStatus, masterDistributorCom: mdCommAmt, masterDistributorComTDS: masterDistTDS, addedBy: commData.users.masterDistributor.id, updatedBy: commData.users.masterDistributor.id }));
                }
                if (commData.wallets.companyWallet) {
                    ...
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.companyAdmin.id, companyId: user.companyId, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - company comm`, amount: amountNumber, comm: companyCommAmt, surcharge: 0, openingAmt: cO, closingAmt: cC, credit: companyNetAmt, debit: wlShortfallAmt + whitelabelTDS, merchantTransactionId, transactionId, paymentStatus, whitelabelComm: companyCommAmt, whitelabelCommTDS: whitelabelTDS, addedBy: commData.users.companyAdmin.id, updatedBy: commData.users.companyAdmin.id }));
                }
                if (commData.wallets.superAdminWallet) {
                    ...
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.superAdmin.id, companyId: 1, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - admin comm`, amount: amountNumber, comm: superAdminCommAmt, surcharge: 0, openingAmt: sO, closingAmt: sC, credit: superAdminNetAmt, debit: saShortfallAmt + superAdminTDS, merchantTransactionId, transactionId, paymentStatus, superadminComm: superAdminCommAmt, superadminCommTDS: superAdminTDS, addedBy: commData.users.superAdmin.id, updatedBy: commData.users.superAdmin.id }));
                }
        ...
        if (isSuccess) {
            ...
            if ([4, 5].includes(user.userRole) && commData.users.companyAdmin) {
                ...
                if (commData.users.distributor && commData.wallets.distributorWallet && user.userRole === 5) {
                    const dW = commData.wallets.distributorWallet, dO = round4(dW.apes2Wallet || 0), dC = round4(dO + distNetAmt - distShortfallAmt);
                    walletUpdates.push(dbService.update(model.wallet, { id: dW.id }, { apes2Wallet: dC, updatedBy: commData.users.distributor.id }));
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.distributor.id, companyId: user.companyId, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - dist comm`, amount: 0, comm: distCommAmt, surcharge: 0, openingAmt: dO, closingAmt: dC, credit: distNetAmt, debit: distShortfallAmt + distributorTDS, merchantTransactionId, transactionId, paymentStatus, distributorCom: distCommAmt, distributorComTDS: distributorTDS, addedBy: commData.users.distributor.id, updatedBy: commData.users.distributor.id }));
                }
                if (commData.users.masterDistributor && commData.wallets.masterDistributorWallet) {
                    const mW = commData.wallets.masterDistributorWallet, mO = round4(mW.apes2Wallet || 0), mC = round4(mO + mdNetAmt - mdShortfallAmt);
                    walletUpdates.push(dbService.update(model.wallet, { id: mW.id }, { apes2Wallet: mC, updatedBy: commData.users.masterDistributor.id }));
                ...
                if (commData.wallets.companyWallet) {
                    const cW = commData.wallets.companyWallet, cO = round4(cW.apes2Wallet || 0), cC = round4(cO + companyNetAmt - wlShortfallAmt);
                    walletUpdates.push(dbService.update(model.wallet, { id: cW.id }, { apes2Wallet: cC, updatedBy: commData.users.companyAdmin.id }));
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.companyAdmin.id, companyId: user.companyId, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - company comm`, amount: 0, comm: companyCommAmt, surcharge: 0, openingAmt: cO, closingAmt: cC, credit: companyNetAmt, debit: wlShortfallAmt + whitelabelTDS, merchantTransactionId, transactionId, paymentStatus, whitelabelComm: companyCommAmt, whitelabelCommTDS: whitelabelTDS, addedBy: commData.users.companyAdmin.id, updatedBy: commData.users.companyAdmin.id }));
                }
                if (commData.wallets.superAdminWallet) {
                    const sW = commData.wallets.superAdminWallet, sO = round4(sW.apes2Wallet || 0), sC = round4(sO + superAdminNetAmt - saShortfallAmt);
                    walletUpdates.push(dbService.update(model.wallet, { id: sW.id }, { apes2Wallet: sC, updatedBy: commData.users.superAdmin.id }));
                    historyPromises.push(dbService.createOne(model.walletHistory, { refId: commData.users.superAdmin.id, companyId: 1, walletType: 'AEPS2', operator: operator?.operatorName || bankIIN, remark: `${remarkText} - admin comm`, amount: 0, comm: superAdminCommAmt, surcharge: 0, openingAmt: sO, closingAmt: sC, credit: superAdminNetAmt, debit: saShortfallAmt + superAdminTDS, merchantTransactionId, transactionId, paymentStatus, superadminComm: superAdminCommAmt, superadminCommTDS: superAdminTDS, addedBy: commData.users.superAdmin.id, updatedBy: commData.users.superAdmin.id }));
                }
                await Promise.all([...walletUpdates, ...historyPromises]);
            } else {
                if (initiatorCredit > 0) walletUpdates.push(dbService.update(model.wallet, { id: wallet.id }, { apes2Wallet: closingAeps2Wallet, updatedBy: req.user.id }));
