...
const createUser = async (req, res) => {
  try {
    ...
    // Auto-generate default Good/Bad schemas based on role (no schemas for Retailer)
    try {
      const role = createdUser.userRole;
      const roleMap = {
        2: 'WU', // Company / Whitelabel admin
        3: 'MD', // Master Distributor
        4: 'DI'  // Distributor
      };
      if (role === 2 || role === 3 || role === 4) {
        const roleName = roleMap[role] || 'UNKNOWN';
        await autoGenerateDefaultSchemas(companyId, createdUser.id, role, roleName);
      }
    } catch (schemaError) {
      console.error('Error auto-generating default schemas for user:', schemaError);
      // Continue user creation even if schema setup fails
    }
