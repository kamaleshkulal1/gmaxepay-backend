...
const creditDebitUserWallet = async (req, res) => {
    try {
        if (req.user.userRole !== 6 || req.user.companyId !== 1) {
            return res.failure({
                message: 'Only authorized employees can access this endpoint'
            });
        }
        ...
        if (!adminUser) {
            return res.failure({ message: 'Superadmin user not found or inactive' });
        }
        ...
        if (actionUpper === 'CREDIT') {
            if (adminOpeningBalance < transferAmount) {
                return res.failure({
                    message: `Insufficient superadmin wallet balance. Available: ${adminOpeningBalance}, Required: ${transferAmount}`
                });
            }
            ...
            adminWalletHistoryData.remark = remarks || `Manual transfer (debit) to user ID: ${userId} - ${transactionId} (initiated by employee ID: ${req.user.id})`;
            ...
            userWalletHistoryData.remark = remarks || `Manual transfer (credit) received from superadmin (initiated by employee ID: ${req.user.id}) - ${transactionId}`;
            ...
            userWalletHistoryData.remark = remarks || `Manual transfer (debit) by employee ID: ${req.user.id} - ${transactionId}`;
            userWalletHistoryData.operator = 'MANUAL_DEBIT';
            userWalletHistoryData.closingAmt = userClosingBalance;
            userWalletHistoryData.credit = 0;
            userWalletHistoryData.debit = transferAmount;

            adminWalletHistoryData.remark = remarks || `Manual transfer (credit) recovered from user ID: ${userId} - ${transactionId} (initiated by employee ID: ${req.user.id})`;
