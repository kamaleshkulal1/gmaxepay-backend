...
const checkOnboardingStatus = async (req, res) => {
    try {
        ...
        const existingUser = await dbService.findOne(model.user, query);
        if (!existingUser) return res.failure({ message: 'User not found or access denied' });
        ...
        console.error('[Employee ZupayAeps] checkOnboardingStatus error:', err);
