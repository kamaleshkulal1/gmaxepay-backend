        ...
        if (mode === 'bank') {
            ...
            if ((payoutHistoryData.status === 'SUCCESS' || payoutHistoryData.status === 'PENDING') && calculatedAmount > 0) {
                ...
                if (commType === 'sur') {
                    ...
                    // Bank Charge Logic (from payoutOperator)
                    let saBankCharge = 0;
                    if (payoutOperator) {
                    ...
                    const saWalletType = walletType; // 'apes1Wallet' or 'apes2Wallet'
                    ...
                    // Super Admin History 1: Surcharge Credit
                    if (saSurcharge > 0) {
                        await dbService.createOne(model.walletHistory, {
                            refId: superAdmin.id,
                            companyId: 1,
                            walletType: saWalletType,
                            operator: operatorName,
                            remark: `${remarkText} - surcharge profit`,
                            amount: saSurcharge,
                            comm: saSurcharge, // It's a commission/profit
                            surcharge: 0,
                            openingAmt: saOpen,
                            closingAmt: saMid,
                            credit: saSurcharge,
                    ...
                    if (saBankCharge > 0) {
                        await dbService.createOne(model.walletHistory, {
                            ...
                            surcharge: 0,
                            openingAmt: saMid,
                            closingAmt: saClose, // Final Balance
                            credit: 0,
                            debit: saBankCharge,
                            transactionId: transactionID,
                            paymentStatus: 'SUCCESS',
...
const checkPayoutStatus = async (req, res) => {
    try {
        ...
        if (existingPayout.payoutType === 'OneKlick' || existingPayout.payoutType === 'oneklick') {
            const response = await oneklick.checkStatus(existingPayout.transactionID);
