...
const getAeps3Reports = async (req, res) => {
    try {
        ...
        if (userRole !== 2) {
            return res.failure({
                message: 'Access denied. Only Company Admin can access AEPS3 reports.'
            });
        }
        ...
        return res.failure({
            message: error.message || 'Unable to retrieve AEPS3 reports'
        });
    }
};
...
const getAeps3TransactionDetailsById = async (req, res) => {
    try {
        ...
        if (existingUser.userRole !== 2) {
             return res.failure({ message: 'Access denied. Only Company Admin can view transaction details.' });
        }
        ...
        if (!transaction) {
            return res.failure({ message: 'Transaction not found or unauthorized' });
        }
        ...
        const data = {
            ...
            reportingUserDetails: {
                companyName: companyDetails.companyName,
                parentName: reportingUserDetails?.name || existingUser.name,
                parentRole: reportingUserDetails?.userRole || existingUser.userRole,
                parentUserId: reportingUserDetails?.userId || existingUser.userId
            },
            transactionDetails: {
                amount: transaction.transactionAmount,
                bankName: bankName,
...
const getCmsReports = async (req, res) => {
    try {
        ...
        if (userRole !== 2) {
            return res.failure({ message: 'Access denied. Only Company Admin can access CMS reports.' });
        }
        ...
        if (dataToFind?.customSearch && typeof dataToFind.customSearch === 'object') {
            ...
            if (customSearch.mobileNo) {
                const searchValue = String(customSearch.mobileNo).trim();
                if (searchValue) {
                    searchConditions.push({
                        mobileNo: {
                            [Op.iLike]: `%${searchValue}%`
            ...
            if (customSearch.mobilePhone) {
                const searchMobile = String(customSearch.mobilePhone).trim();
                if (searchMobile) {
                    userSearchFields.push({
                        mobileNo: {
                            [Op.iLike]: `%${searchMobile}%`
                        }
                    });
                }
            }
